...
test('carousel uses lesson metadata and preserves resources without completing tasks', () => {
  ...
  assert.ok(component.includes('hasViewedAllSlides'))
  assert.ok(component.includes('All slides viewed'))
  assert.ok(component.includes('View full lesson'))
  assert.ok(!component.includes('aria-pressed={readAll}'))
