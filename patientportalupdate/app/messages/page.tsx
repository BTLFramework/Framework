...
export default function MessagesPage() {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        setLoading(true)
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/messages/patient/${patientId}`)
        ...
        if (result.success) {
          ...
          const markAsRead = async () => {
            try {
              await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/messages/patient/${patientId}/mark-read`, {
                method: 'PATCH'
              })
            } catch (error) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/messages/reply`, {
        method: 'POST',
        headers: {
